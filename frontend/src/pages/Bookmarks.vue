<template>
  <div class="h-[calc(100vh-4rem)] overflow-auto p-6 lg:p-8">
      ...
      <div
        ...
        class="flex items-center gap-3 mb-4 px-4 py-2.5 rounded-xl border border-border/50"
        style="background-color: hsl(var(--card))"
      >
        <span class="text-xs font-medium text-muted-foreground">{{ t('bookmarks.selected', { count: selectedIds.size }) }}</span>
...
const allSelectedOnPage = computed(() => {
  const pageIds = displayItems.value.map((bm: any) => bm.id)
  return pageIds.length > 0 && pageIds.every((id: number) => selectedIds.value.has(id))
})

const toggleSelectAll = () => {
  if (allSelectedOnPage.value) {
    displayItems.value.forEach((bm: any) => selectedIds.value.delete(bm.id))
  } else {
    displayItems.value.forEach((bm: any) => selectedIds.value.add(bm.id))
  }
...
const handleBatchDelete = async () => {
  ...
  if (!confirm(t('bookmarks.batchDeleteConfirm', { count: ids.length }))) return
  try {
    await bookmarks.batchDelete(ids)
    clearSelection()
    load()
  } catch (_) {}
}
...
watch(searchQuery, () => {
  if (searchMode.value === 'normal') { page.value = 1; clearSelection(); load() }
})
watch(searchMode, () => {
